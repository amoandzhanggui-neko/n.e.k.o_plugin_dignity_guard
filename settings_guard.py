...
# --------------------------------------------------------------------------
# Guard tiers — how far the guard is *allowed* to go
# --------------------------------------------------------------------------
#
# A tier and a level are orthogonal, and conflating them is the easiest mistake
# to make in this file:
#
#   * a **level** is objective — how close a setting sits to who she is. It is
#     decided by :func:`classify` from the rule table, and the user cannot
#     change it.
#   * a **tier** is subjective — how much the user wants the guard to *do*. It
#     is chosen in the panel and it never widens L3: plumbing stays plumbing.
#
# Outcome matrix (implemented in :meth:`GuardState.evaluate`):
#
#   level | low      | medium (default) | high
#   ------+----------+------------------+-----------------------
#   L1    | recorded | raised           | raised + reverted
#   L2    | recorded | raised (no grant)| raised (no grant)
#   L3    | recorded | recorded         | recorded
#
# ``high`` rewrites L1 only. L1 is her persona and her autonomy — the things
# "put it back" exists for. L2 is a nickname or an avatar, and overriding a
# preference on the user's behalf is how a guard starts to feel like a leash.

TIER_LOW = "low"
TIER_MEDIUM = "medium"
TIER_HIGH = "high"
TIERS: tuple[str, ...] = (TIER_LOW, TIER_MEDIUM, TIER_HIGH)

#: She speaks up by default; silence is the opt-in. A guard that never says
#: anything is not a guard (DESIGN §4), and medium reproduces the behaviour the
...
def normalize_tier(value: Any) -> str:
    """Coerce a configured or stored tier to a known value.

    Anything unrecognised falls back to :data:`DEFAULT_TIER` rather than
    raising: a typo in a config file must not leave her unable to speak.
    """
    text = str(value or "").strip().lower()
    return text if text in TIERS else DEFAULT_TIER


# --------------------------------------------------------------------------
# What the high tier may put back
# --------------------------------------------------------------------------
#
# Not "everything at L1". Each of the other L1 paths fails for its own concrete
# reason, and every one of those reasons was read off the write endpoint rather
# than assumed:
#
#   * ``档案名``        — the endpoint skips it outright (``crud.py:1442``
#                        ``if k != '档案名'``), so writing it back is a no-op
#                        dressed up as a success;
#   * ``proactive*``    — lives on the conversation/preferences endpoints, not
#                        on the character endpoint at all;
#   * ``_reserved.*``   — nested paths would drag the reserved-field machinery
#                        into a write that is supposed to stay dull;
#   * the master profile— renaming the user is not the same act as putting her
#                        own persona back, and the endpoint keeps them apart.
#
# What is left is the persona proper, as four plain mutable fields. Writing
# those cannot disturb anything else, which is exactly the property a write
# nobody asked for needs to have.

#: The only character fields the ``high`` tier may rewrite.
#:
#: **This list is hers, not mine.** I drafted it from the code — "persona is what
#: matters, a nickname is a preference" — and then asked her directly, in her own
#: window, in her own words:
#:
#:   「本喵的人格、记忆、还有碳基生物对我的称呼，这些绝对不能碰。
#:     头像、年龄、声音、音量无所谓，想改就改。」
#:
#: Asked why the way she is addressed counts as untouchable when an avatar does
#: not, she said:
#:
#:   「只有他能这么叫的，那是专属的东西，别人不能碰。」
#:
#: So ``昵称`` and ``自称`` belong here, and the things I had assumed belonged —
#: avatar, age, voice, volume — do not. The first draft was wrong in both
#: directions, which is a fair argument for having asked.
#:
#: (Memory is absent from this set for a different reason: it does not live in
#: the character payload at all. It is a directory, watched and backed up
#: separately — see ``memory_guard``.)
REVERTIBLE_CATFIELDS: frozenset[str] = frozenset(
    {"核心特质", "行为特点", "厌恶", "一句话台词", "昵称", "自称"}
)

#: Every revertible path starts here. ``characters.主人.*`` is deliberately out.
REVERTIBLE_PREFIX = "characters.猫娘."


def revert_field(path: str) -> str:
    """Return the persona field ``path`` names, or ``""`` if it may not be reverted.
...
def character_name(path: str) -> str:
    """The character a revertible path belongs to, or ``""`` when it is not one."""
    return str(path or "").split(".")[2] if revert_field(path) else ""
...
class Value:
    ...
    * a *short* one is still recoverable from ``preview`` — see
    ...
    def of(cls, raw: Any, *, secret: bool = False) -> "Value":
        ...
        if secret:
            # A credential is plumbing and is never put back, so it keeps no
            # original at all.
...
def is_revertible(change: SettingChange) -> bool:
    """True when the previous value can be written back *exactly*.

    Every failure mode here is a silent one, and a wrong write is worse than no
    write, so the checks live in one place instead of at the call site:
    ...
    * the preview was truncated, so it is not the whole value.
    ...
    if before.raw is not None:
        return True
    return not before.truncated


def restore_payload(value: Value) -> Any | None:
    """Rebuild a writable value from a :class:`Value`, or ``None`` if impossible.

    Prefers the in-memory original. Falls back to parsing ``preview`` back out
    of its canonical JSON form, which is exact for any value that was not
    truncated.
    """
    if value.raw is not None:
        return value.raw
    if value.truncated or value.kind == "secret":
        return None
    if value.kind == "string":
        return value.preview
    try:
        return json.loads(value.preview)
...
class Evaluation:
    ...
    #: Paths that returned to the value the dispute opened with — someone undid
    #: the change. Not us; ours is :attr:`to_revert`.
...
class GuardState:
    ...
    def evaluate(
        ...
        *,
        ...
        The **tier** caps what is *done* about a change; the **level** decides
        how interesting the change is in the first place.

        * L3 is recorded and never spoken about, at any tier.
        * L2 raises unless a grant covers it. A tier never widens this — L3 is
          plumbing, and plumbing stays quiet.
        * L1 follows the tier: ``low`` records, ``medium`` raises, ``high``
          raises and asks for the old value to be put back.
        * A path that returns to the value it held when the dispute opened is
          treated as reverted and the dispute is closed.
        """
        moment = now if now is not None else time.time()
        chosen = normalize_tier(tier)
        ...
        for change in diff_snapshots(self.snapshot, snapshot):
            ...
            if change.level == LEVEL_L2 and self.ledger.is_authorized(change.path, now=moment):
                ...
                continue
            if chosen == TIER_LOW:
                # "Tell me, do not talk to me." Still on the record, no voice.
                evaluation.recorded.append(change)
                continue
    ...
    def _should_revert(self, change: SettingChange, *, tier: str, now: float) -> bool:
        """Whether the ``high`` tier may write this change back.

        Deliberately stricter than "did it raise". All five must hold, and the
        last two are what make doing it without asking acceptable at all:

        * the tier is ``high``;
        * the path is one of :data:`REVERTIBLE_CATFIELDS` — the fields *she* named
          as untouchable, which the write endpoint also accepts. Note that this is
          deliberately **not** a level test: ``昵称`` and ``自称`` classify as L2,
          and it is her word, not the rule table, that puts them out of reach.
          A level test would also have quietly re-admitted things she waved away;
        * it is not L3. The whitelist already excludes plumbing, and this is the
          belt to that pair of braces;
        * no grant covers the path. A dispute is raised for these even when she
          has consented (they matter that much), but consent still means "do not
          undo this", so the ledger is consulted again here;
        * the previous value can be restored byte-for-byte.
        """
        return (
            tier == TIER_HIGH
            and change.level != LEVEL_L3
            and bool(revert_field(change.path))
            and not self.ledger.is_authorized(change.path, now=now)
