...
* :class:`MainServerClient` — the only place in this plugin that touches the
  network. It talks to ``http://127.0.0.1:<port>``; it mostly reads, plus two
  methods that write and are named as such.
...
Writing is confined to :meth:`MainServerClient.fetch_characters_raw` and
:meth:`MainServerClient.put_catgirl`. The plugin still cannot *intercept* a
config write (see DESIGN.md §2) — but the ``high`` tier can undo one afterwards,
and that undo is the only reason a write path exists here at all.
"""
...
class MainServerClient:
    ...
    # -- the one write ------------------------------------------------------
    #
    # Everything above only reads. These two are the entire write surface, and
    # their shape is deliberate: read the full payload, change one field, send
    # the whole payload back. The endpoint has no partial-update mode, so a
    # caller that skips the read is a caller that deletes things.

    async def fetch_characters_raw(self) -> dict[str, Any]:
        """Read the character payload *as stored*.

        ``?language=zh-CN`` is not decoration. Without it the endpoint
        translates ``昵称`` / ``性别`` on the way out (``crud.py:677-699``), and
        this payload is the base handed straight back to :meth:`put_catgirl` —
        a translated base would write the translation over her own words.
        """
        payload = await self._get_json(f"{CHARACTERS_PATH}?language=zh-CN")
        if not isinstance(payload, Mapping):
            raise MainServerUnreachable(CHARACTERS_PATH, "payload is not an object")
        return dict(payload)

    async def put_catgirl(self, name: str, body: Mapping[str, Any]) -> dict[str, Any]:
        """Replace one character's profile. ``body`` must be the complete field set.

        The endpoint deletes every field it does not receive
        (``crud.py:1432-1438``), so a partial update is a destructive one.
        """
        endpoint = f"{CHARACTERS_PATH}/catgirl/{quote(str(name), safe='')}"
        try:
            async with self._make_client() as client:
...
class SettingsWatcher:
    ...
    def __init__(
        ...
        *,
        ...
        #: Mirrors ``DignityGuardPlugin._tier``. The plugin updates it whenever
        #: the user changes the tier, so ``poll`` needs no extra argument.
